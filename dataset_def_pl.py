...
class StickerDataset(torch.utils.data.Dataset):
    ...
    @staticmethod
    def process_images(path="test_images/IMG_3774.jpg"):
        p, d, dist_maps = load_camera_calibration()
        image_og = cv.imread(path)
        # Undistorts images with the maps calculated in load_camera_calibration()
        image_og = cv.remap(image_og, dist_maps[0], dist_maps[1], cv.INTER_LINEAR)
    ...
    def __getitem__(self, idx):
        ...
        # img, img_rz = self.process_images(self.images[idx])
        # get the image name
...
class StickerData(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        return DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers,
                          collate_fn=collate_fn, pin_memory=True, persistent_workers=True, drop_last=True)
